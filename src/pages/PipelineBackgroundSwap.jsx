...
const PipelineBackgroundSwap = () => {
  ...
  const handleRun = useCallback(async () => {
    ...
    setError("");
    setIsRunning(true);
    const effectiveRefs = refImages.length > 0
      ? refImages
      : [{ id: "no-ref", name: "无参考图", url: null }];
    const totalTasks = mainImages.length * effectiveRefs.length;
    setProgress({ done: 0, total: totalTasks });
    const seed = buildResultsSeed(mainImages, effectiveRefs);
    resultsSeedRef.current = seed;
    setResults(seed);

    for (const mainItem of mainImages) {
      for (const refItem of effectiveRefs) {
        const resultId = `${mainItem.id}-${refItem.id}`;
        updateResult(resultId, { status: "running", error: null });
        ...
        try {
          const resp = await apiFetch(`/api/multi_image_generate`, {
            ...
            body: JSON.stringify({
              ...
              images: refItem.url ? [mainItem.url, refItem.url] : [mainItem.url],
              temperature: 0.7,
              size,
          ...
          updateResult(resultId, { status: "success", outputUrl });
        } catch (err) {
          updateResult(resultId, { status: "error", error: err?.message || String(err) });
        }
    ...
    setIsRunning(false);
  }, [apiFetch, aspectRatio, hasReadyInputs, isRunning, mainImages, prompt, refImages, size, updateResult]);
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      <header className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/60">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold">流水线换背景</h1>
            <p className="text-xs text-slate-400">批量替换主图背景，保持主体与风格一致</p>
          </div>
        </div>
      ...
      <div className="flex-1 px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-semibold">背景参考图添加区域</h2>
            ...
            {refImages.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center border border-dashed border-slate-700 rounded-xl py-10 text-slate-500">
                <ImagePlus className="w-8 h-8 mb-2" />
                <div className="text-sm">拖拽或点击上传背景参考图</div>
                <div className="text-xs text-slate-600 mt-1">支持多张，建议背景清晰、光线自然</div>
              </div>
            )}
          </section>
        </div>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-xs text-slate-500">用于增强换背景指令，默认已经适配常见场景</p>
            </div>
              ...
              <button
                onClick={() => {
                  results.forEach((item) => {
                    ...
                    link.download = `bg-swap-${item.id}.png`;
                    link.click();
                  });
                }}
                className="text-xs px-3 py-1.5 rounded-md bg-slate-800 border border-slate-700 hover:border-purple-500 hover:text-white transition-colors"
              ...
              {results.map((item) => (
                <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="text-[11px] text-slate-500">换背景结果</div>
                        {item.outputUrl && (
                          <button
                            onClick={() => {
                              const link = document.createElement("a");
                      ...
                      <div className="relative">
                        ...
                        {item.outputUrl ? (
                          <button
                            ...
                            title="点击放大预览"
                          >
                            <img src={item.outputUrl} alt="换背景结果" className="w-full h-full object-cover" />
                          </button>
                        ) : (
                          <div className="w-full h-40 rounded-lg border border-dashed border-slate-800 flex items-center justify-center text-xs text-slate-600">
...
export default PipelineBackgroundSwap;
