      ...
      <header className="px-4 sm:px-6 py-4 border-b border-slate-800 bg-slate-900/60">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">单图生成短视频，默认时长 3 秒</p>
          </div>
        </div>
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-xs text-slate-500">用于描述动作与镜头，默认已经适配常见场景</p>
            </div>
          ...
          <div className="mt-3 text-xs text-slate-500">视频时长：{DEFAULT_DURATION} 秒（默认）</div>
          <textarea
            value={prompt}
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        {item.outputUrl ? (
                          <button
                            type="button"
                            onClick={() => setPreviewVideo(item.outputUrl)}
                            className="block w-full h-40 rounded-lg border border-slate-800 overflow-hidden"
                            title="点击放大预览"
                          >
                            <video src={item.outputUrl} className="w-full h-full object-cover" muted loop playsInline />
                          </button>
                        ) : (
