...
const DEFAULT_HIGHLIGHT_COLOR = "#FF4D6D";
const DEFAULT_HIGHLIGHT_COLORS = ["#FF4D6D", "#FDE047", "#60A5FA"];
const DEFAULT_BOLD_COLORS = ["#FFFFFF", "#F97316", "#22C55E"];
// Keep defaults in sync with bananaflow/workflows/overlaytext.json (font_size 50, bold_strength 2).
...
const buildResultsSeed = (items) =>
  ...
  const [highlightItems, setHighlightItems] = useState([
    { text: "", color: DEFAULT_HIGHLIGHT_COLORS[0] },
    { text: "", color: DEFAULT_HIGHLIGHT_COLORS[1] },
    { text: "", color: DEFAULT_HIGHLIGHT_COLORS[2] },
  ]);
  const [boldItems, setBoldItems] = useState([
    { text: "", color: DEFAULT_BOLD_COLORS[0] },
    { text: "", color: DEFAULT_BOLD_COLORS[1] },
    { text: "", color: DEFAULT_BOLD_COLORS[2] },
  ]);
  ...
  const normalizedTextColor = useMemo(
    () => normalizeHexColor(textColor, DEFAULT_TEXT_COLOR),
    [textColor],
  );
  const normalizedBgColor = useMemo(
    () => normalizeHexColor(bgColor, DEFAULT_BG_COLOR),
    [bgColor],
  );
  const normalizedHighlightItems = useMemo(
    () =>
      highlightItems.map((item, idx) => ({
        text: item.text,
        color: normalizeHexColor(item.color, DEFAULT_HIGHLIGHT_COLORS[idx] || DEFAULT_HIGHLIGHT_COLOR),
      })),
    [highlightItems],
  );
  const normalizedBoldItems = useMemo(
    () =>
      boldItems.map((item, idx) => ({
        text: item.text,
        color: normalizeHexColor(item.color, DEFAULT_BOLD_COLORS[idx] || DEFAULT_TEXT_COLOR),
      })),
    [boldItems],
  );
  ...
  const updateHighlightItem = useCallback((index, patch) => {
    setHighlightItems((prev) => prev.map((item, i) => (i === index ? { ...item, ...patch } : item)));
  }, []);

  const updateBoldItem = useCallback((index, patch) => {
    setBoldItems((prev) => prev.map((item, i) => (i === index ? { ...item, ...patch } : item)));
  }, []);
      ...
      try {
        const overlayResp = await apiFetch(`/api/overlaytext`, {
          ...
          body: JSON.stringify({
            ...
            use_bg_color: useBgColor,
            highlight_texts: normalizedHighlightItems.map((item) => item.text?.trim() || ""),
            highlight_colors: normalizedHighlightItems.map((item) => item.color),
            bold_texts: normalizedBoldItems.map((item) => item.text?.trim() || ""),
            bold_colors: normalizedBoldItems.map((item) => item.color),
            font_size: fontSize,
        ...
        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div>
              ...
              <p className="text-xs text-slate-500">文案将自动拼接到提示词，支持多行输入</p>
            </div>
          ...
          <div className="mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="text-xs text-slate-400 space-y-3">
                  <span className="block">高亮内容（最多 3 条）</span>
                  {normalizedHighlightItems.map((item, idx) => (
                    <div key={`highlight-${idx}`} className="grid grid-cols-1 sm:grid-cols-[1fr,160px] gap-2">
                      <input
                        value={highlightItems[idx]?.text || ""}
                        onChange={(e) => updateHighlightItem(idx, { text: e.target.value })}
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={item.color}
                          onChange={(e) => updateHighlightItem(idx, { color: e.target.value })}
                          className="h-9 w-12 rounded border border-slate-700 bg-slate-950"
                        />
                        <input
                          value={highlightItems[idx]?.color || ""}
                          onChange={(e) => updateHighlightItem(idx, { color: e.target.value })}
                          placeholder={DEFAULT_HIGHLIGHT_COLORS[idx] || DEFAULT_HIGHLIGHT_COLOR}
                          className="flex-1 rounded-lg bg-slate-950 border border-slate-800 text-sm text-slate-200 px-3 py-2 focus:outline-none focus:border-purple-500"
                        />
                      </div>
                    </div>
                  ))}
                  <div className="text-[11px] text-slate-500">高亮指文字颜色变化，不使用底色。</div>
                </div>

                <div className="text-xs text-slate-400 space-y-3">
                  <span className="block">加粗内容（最多 3 条）</span>
                  {normalizedBoldItems.map((item, idx) => (
                    <div key={`bold-${idx}`} className="grid grid-cols-1 sm:grid-cols-[1fr,160px] gap-2">
                      <input
                        value={boldItems[idx]?.text || ""}
                        onChange={(e) => updateBoldItem(idx, { text: e.target.value })}
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="color"
                          value={item.color}
                          onChange={(e) => updateBoldItem(idx, { color: e.target.value })}
                          className="h-9 w-12 rounded border border-slate-700 bg-slate-950"
                        />
                        <input
                          value={boldItems[idx]?.color || ""}
                          onChange={(e) => updateBoldItem(idx, { color: e.target.value })}
                          placeholder={DEFAULT_BOLD_COLORS[idx] || DEFAULT_TEXT_COLOR}
                          className="flex-1 rounded-lg bg-slate-950 border border-slate-800 text-sm text-slate-200 px-3 py-2 focus:outline-none focus:border-purple-500"
                        />
                      </div>
                    </div>
            ...
            <div className="space-y-3">
              <label className="text-xs text-slate-400 space-y-2">
                <span className="block">文案是否需要底色</span>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setUseBgColor((prev) => !prev)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="text-xs text-slate-400 space-y-2">
                  <span className="block">文字颜色（色号）</span>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                ...
                <label className="text-xs text-slate-400 space-y-2">
                  <span className="block">底色颜色（色号）</span>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      value={normalizedBgColor}
