        ...
        return _jsonrpc_ok(
            req_id,
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"Unsupported tool: {name}. Recovery: call {EXPORT_FFMPEG_TOOL_NAME}.",
                    }
                ],
            },
        )
    try:
        output = execute_export_ffmpeg_tool(arguments=arguments, plan_lookup=None)
        return _jsonrpc_ok(
            req_id,
            {
                "isError": False,
        ...
        return _jsonrpc_ok(
            req_id,
            {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"Tool call failed: {e}. Recovery: validate plan_id/plan payload and output directory.",
                    }
                ],
            },
        )
