...
def build_intake_prompt(payload: Dict[str, Any]) -> str:
    return (
        ...
        "如果输入中提供了 script_table / script_rows，它们是更强的镜头来源，应优先保留其中的镜号、时长、台词、音效和画面信息。\n"
        "如果存在 script_rows，则不得脱离 script_rows 自行虚构新的故事主题、角色关系、场景母题或镜头顺序。\n"
...
def build_entity_design_prompt(payload: Dict[str, Any]) -> str:
    return (
        ...
        "如果输入里带有 script_table / script_rows，请从镜头表中抽取角色、主体、场景，并以它为准整理命名。\n"
        "如果存在 script_rows，则只允许抽取 rows 中出现或可直接推断出的角色、主体、场景，不要引入 rows 之外的故事设定。\n"
        ...
        "例如不要只写“某个场景，气氛紧张”，而要写成包含空间形态、主光源、地面/墙面状态、空气介质、色彩对比和整体氛围的完整环境描述。\n"
        "不要输出 markdown。\n"
        f"输入：\n{_json(payload)}"
    )
...
def build_scene_outline_prompt(payload: Dict[str, Any]) -> str:
    return (
        ...
        "如果输入里有 script_rows，请按镜头表中真实空间和转场进行 scene 聚合，不要按动作或事件拆场景。\n"
        "如果存在 script_rows，则 scene 列表必须完全服从 rows 中的空间变化和镜头顺序，不要引入 rows 之外的主题化场景。\n"
        ...
        "例如可以写空间名或环境名，不要写“秘密接头”“真相揭露”“失控追逐”这种动作或事件标题。\n"
        ...
        "如果输入里有 script_rows，请确保每个 shot row 都能归属到某个 scene，并且 scene 顺序覆盖镜头顺序。\n"
        "不要输出 shots。\n"
        f"输入：\n{_json(payload)}"
    )
...
def build_shot_design_prompt(payload: Dict[str, Any]) -> str:
    return (
        ...
        "如果输入里有 script_table / script_rows，则它是权威镜头清单：优先保留 shot_no、camera、duration、dialogue、sound、reference 信息，再整理到当前 storyboard 结构中。\n"
        "如果存在 script_rows，则不得改写核心镜头内容，不得替换为不相关题材，不得引入 rows 中不存在的母题或场景。\n"
        ...
        "如果某行镜头表写了“画外音”，应优先归入 voiceover；明确角色说话应拆到 dialogues。\n"
        "如果镜头表写了固定/运动、景别，请合并到 camera 字段，不要丢失。\n"
