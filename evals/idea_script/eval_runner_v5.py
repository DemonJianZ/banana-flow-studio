...
def _edit_plan_quality(out: Any) -> Dict[str, Any]:
    plans = list(getattr(out, "edit_plans", []) or [])
    clip_count_total = 0
    missing_primary_asset_count = 0
    best_primary_count = 0
    fallback_primary_count = 0
    primary_bucket_distribution: Dict[str, int] = {"best_match": 0, "partial_match": 0, "fallback": 0, "missing": 0}
    for plan in plans:
        for track in list(getattr(plan, "tracks", []) or []):
            for clip in list(getattr(track, "clips", []) or []):
                clip_count_total += 1
                primary = getattr(clip, "primary_asset", None)
                if primary is None:
                    missing_primary_asset_count += 1
                    primary_bucket_distribution["missing"] = primary_bucket_distribution.get("missing", 0) + 1
                    continue
                bucket = str(getattr(primary, "bucket", "fallback") or "fallback")
                primary_bucket_distribution[bucket] = primary_bucket_distribution.get(bucket, 0) + 1
                if bucket == "best_match":
                    best_primary_count += 1
                if bucket == "fallback":
                    fallback_primary_count += 1
    missing_primary_asset_rate = round(float(missing_primary_asset_count) / max(clip_count_total, 1), 3)
    best_match_primary_rate = round(float(best_primary_count) / max(clip_count_total, 1), 3)
    fallback_primary_rate = round(float(fallback_primary_count) / max(clip_count_total, 1), 3)
    return {
        "edit_plan_count": len(plans),
        "clip_count_total": clip_count_total,
        "missing_primary_asset_count": missing_primary_asset_count,
        "missing_primary_asset_rate": missing_primary_asset_rate,
        "best_match_primary_rate": best_match_primary_rate,
        "fallback_primary_rate": fallback_primary_rate,
...
def main() -> int:
    ...
    for idx, case in enumerate(cases, start=1):
        ...
        edit_plan_quality = _edit_plan_quality(out)
        ...
        for key, value in dict(edit_plan_quality.get("primary_bucket_distribution", {}) or {}).items():
            if key not in primary_bucket_distribution:
                primary_bucket_distribution[key] = 0
            primary_bucket_distribution[key] += int(value or 0)
        ...
        item = {
            ...
            "edit_plan_warning": bool(getattr(out, "edit_plan_warning", False)),
            "edit_plan_warning_reason": getattr(out, "edit_plan_warning_reason", None),
            ...
            "edit_plan_quality": edit_plan_quality,
        }
        results.append(item)
    ...
    summary = {
        ...
        "edit_plan_warning_rate": round(
            sum(1 for r in results if bool(r.get("edit_plan_warning"))) / max(total, 1), 3
        ),
        "missing_primary_asset_rate": round(
            sum(float((r.get("edit_plan_quality") or {}).get("missing_primary_asset_rate", 0.0)) for r in results) / max(total, 1),
            3,
        ),
        "best_match_primary_rate": round(
            sum(float((r.get("edit_plan_quality") or {}).get("best_match_primary_rate", 0.0)) for r in results) / max(total, 1),
            3,
        ),
        "fallback_primary_rate": round(
            sum(float((r.get("edit_plan_quality") or {}).get("fallback_primary_rate", 0.0)) for r in results) / max(total, 1),
            3,
        ),
